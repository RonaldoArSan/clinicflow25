...
const defaultClinicSettings: ClinicSettings = {
  ...
  footer:
    "Esta clínica segue os protocolos de segurança estabelecidos pela ANVISA",
...
interface ClinicContextType {
  ...
  error: string | null;
  saveSettings: (
    newSettings: Partial<ClinicSettings>
  ...
  getDocumentFooter: () => string;
  getFullAddress: () => string;
  getTechnicalManagerInfo: () => any;
}
...
export const ClinicProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const exportSettings = () => {
    ...
    link.href = url;
    link.download = `configuracoes-clinica-${
  ...
  const importSettings = (file: File) => {
    return new Promise<{ success: boolean; message: string }>((resolve) => {
      ...
      reader.onerror = () => {
        resolve({
          success: false,
          message: "Erro ao ler o arquivo de configurações.",
        });
      };
  ...
  const getFullAddress = () => {
    return `${settings.address}, ${settings.number}${
      settings.complement ? `, ${settings.complement}` : ""
    } - ${settings.district}, ${settings.city}/${settings.state} - CEP: ${
      settings.cep
    }`;
  };
