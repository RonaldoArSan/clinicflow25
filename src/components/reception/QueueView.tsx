...
const QueueView: React.FC<QueueViewProps> = ({
  ...
  const inServiceCount = queue.filter(item => item.status === 'in_service').length;
  const completedCount = queue.filter(item => item.status === 'completed').length;
  const averageWaitTime = queue.length > 0 
    ? Math.round(queue.reduce((acc, item) => acc + item.waitTime, 0) / queue.length)
    : 0;
  ...
  const updateItemStatus = (item: QueueItem, newStatus: string) => {
  ...
  return (
    <div className="space-y-6">
      {/* Cabeçalho */}
      <div className={`${darkMode ? 'bg-gradient-to-r from-orange-900/20 to-red-900/20 border-orange-800' : 'bg-gradient-to-r from-orange-50 to-red-50 border-orange-200'} rounded-lg border p-6`}>
        <div className="flex items-center space-x-3">
          <Timer className={`w-8 h-8 ${darkMode ? 'text-orange-400' : 'text-orange-600'}`} />
          <div>
            <h2 className={`text-2xl font-bold ${darkMode ? 'text-orange-300' : 'text-orange-700'}`}>
              Fila de Espera
            </h2>
            <p className={`${darkMode ? 'text-orange-400/80' : 'text-orange-600/80'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className={`${darkMode ? 'bg-yellow-900/20 border-yellow-800' : 'bg-yellow-50 border-yellow-200'} border rounded-lg p-4`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-sm font-medium ${darkMode ? 'text-yellow-400/80' : 'text-yellow-600/80'}`}>
                Aguardando
              </p>
            ...
            <Timer className={`w-8 h-8 ${darkMode ? 'text-yellow-400' : 'text-yellow-600'} opacity-60`} />
          </div>
        </div>
        ...
        <div className={`${darkMode ? 'bg-green-900/20 border-green-800' : 'bg-green-50 border-green-200'} border rounded-lg p-4`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-sm font-medium ${darkMode ? 'text-green-400/80' : 'text-green-600/80'}`}>
                Em Atendimento
              </p>
            ...
            <Activity className={`w-8 h-8 ${darkMode ? 'text-green-400' : 'text-green-600'} opacity-60`} />
          </div>
        </div>
        ...
        <div className={`${darkMode ? 'bg-blue-900/20 border-blue-800' : 'bg-blue-50 border-blue-200'} border rounded-lg p-4`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-sm font-medium ${darkMode ? 'text-blue-400/80' : 'text-blue-600/80'}`}>
                Concluídos
              </p>
            ...
            <CheckCircle className={`w-8 h-8 ${darkMode ? 'text-blue-400' : 'text-blue-600'} opacity-60`} />
          </div>
        </div>

        <div className={`${darkMode ? 'bg-purple-900/20 border-purple-800' : 'bg-purple-50 border-purple-200'} border rounded-lg p-4`}>
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-sm font-medium ${darkMode ? 'text-purple-400/80' : 'text-purple-600/80'}`}>
                Tempo Médio
              </p>
            ...
            <Clock className={`w-8 h-8 ${darkMode ? 'text-purple-400' : 'text-purple-600'} opacity-60`} />
          </div>
        </div>
      </div>
      ...
      <div className={`${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-lg border`}>
        <div className="p-4 border-b border-gray-200 dark:border-gray-700">
          <h3 className={`text-lg font-semibold ${darkMode ? 'text-gray-200' : 'text-gray-800'}`}>
            Pacientes na Fila ({filteredQueue.length})
          </h3>
        </div>
        ...
        <div className="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div className="flex space-x-2">
                    ...
                    {item.status === 'waiting' && index < filteredQueue.length - 1 && (
                      <button
                        className="bg-gray-600 text-white p-2 rounded hover:bg-gray-700 transition-colors"
          ...
          {filteredQueue.length === 0 && (
            <div className="p-8 text-center">
              <Timer className={`w-12 h-12 mx-auto mb-4 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`} />
