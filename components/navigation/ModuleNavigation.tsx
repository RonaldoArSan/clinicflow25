...
import { hasModuleAccess, ModuleAccess } from '../../types/user';
...
export default function ModuleNavigation({ 
  ...
  // Filtrar módulos baseado nas permissões do usuário usando a nova hierarquia
  const availableModules = MODULES.filter(module => 
    currentUser && hasModuleAccess(currentUser.role, module.id as ModuleAccess)
  );
              ...
              <div className="flex items-center space-x-2">
                ...
                <span className={`px-2 py-1 rounded text-xs font-medium ${
                  ...
                  {currentUser.role === 'admin' ? '🔧 Administrador' :
                   currentUser.role === 'medical' ? '👨‍⚕️ Médico/Profissional' :
                   currentUser.role === 'reception' ? '🏥 Recepção' :
                   currentUser.role === 'financial' ? '💰 Financeiro' : '👁️ Visualizador'}
                </span>
              </div>
              
              <div className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>
                {availableModules.length} módulo{availableModules.length !== 1 ? 's' : ''} disponível{availableModules.length !== 1 ? 'eis' : ''}
              </div>
            </div>
          </div>
        )}
